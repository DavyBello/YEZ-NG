...
import { LOGIN_MANAGER_MUTATION } from '../lib/backendApi/mutations'
  ...
  class WrappedComponent extends React.Component {
    static async getInitialProps(context, apolloClient) {
      ...
      if (loggedInUser.centerManager) {
        ...
        const target = await context.query.from || `/manager`;
    ...
    onCompleted = (data) => {
      ...
      const {jwt, name: {last}} = data.loginCenterManager
      ...
      this.props.client.resetStore().then(() => {
        const target = this.props.url.query.from || `/manager`;
        redirect({}, target)
      })
    }
    ...
    onError = (error) => {
      ...
      error.graphQLErrors.forEach(error=>{
        switch(error.message) {
          ...
          break;
          case `phone/centerManager not found`:
          toast("Incorrect Username/password", {...TOAST_STYLE.fail});
          break;
          default:
    ...
    render() {
      ...
      return (
        <div>
          ...
          <Mutation mutation={LOGIN_MANAGER_MUTATION}
            onCompleted={this.onCompleted}
            onError={this.onError}>
            {(loginCenterManager, {data, error}) => (
              <Child {...this.props} loginCenterManager={loginCenterManager}/>
            )}
            </Mutation>
            <ToastContainer />
        </div>
