...
import {MANAGER_CANDIDATE_BY_ID_EDUCATION_QUERY} from '../../../../../lib/backendApi/queries'
...
export default class extends Component {
  ...
  render(){
    return (
      <Card>
        <Query query={MANAGER_CANDIDATE_BY_ID_EDUCATION_QUERY} variables={{ id: this.props.id}}>
          {({loading, error, data}) => {
            if (loading)
              return <Loading />;
            if (error)
