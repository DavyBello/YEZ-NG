...
export default class NewMessage extends Component {
  ...
  doUpdate = (e, runMutation) => {
    ...
    if (this.state.title && this.state.message){
      ...
      //   centerManager: this.props.centerManager._id,
  ...
  onCompleted = ({createCenterManagerMessage: {record: {title}}}) => {
  ...
  onError = (error) => {
    ...
    error.graphQLErrors.forEach(error=>{
      switch(error.message) {
        ...
        break;
        case `E11000 duplicate key error collection: ktt-backend.centerManagers index: username_1 dup key: { : "${this.state.username}" }`:
        toast("This Username is taken", {...TOAST_STYLE.fail});
        break;
        case `E11000 duplicate key error collection: ktt-backend.centerManagers index: email_1 dup key: { : "${this.state.email}" }`:
        ...
        <Col xs="12" sm="12" md="12">
          <Card className="card-accent-teal">
            <CardHeader className="h4" style={{fontWeight: '300'}}>
              ...
              <Label className="switch switch-lg switch-text switch-teal float-right mb-0">
              ...
              <CardBody>
                <Row style={{padding: '30px'}}>
                  <Mutation mutation={CREATE_COMPANY_MESSAGE_MUTATION} onCompleted={this.onCompleted} onError={this.onError}>
                    {(createCenterManagerMessage, {data, error}) => (
                      <Form action="" method="post" className="form-horizontal" style={{width: '100%'}}>
                        ...
                        <div style={{width: '100%'}}>
                          <Button className="float-right" color="dark" onClick={e=>this.doUpdate(e, createCenterManagerMessage)} style={{marginBottom: '1rem'}}>Send Message</Button>
                        </div>
                      </Form>
                    )}
                  </Mutation>
